...
export const addextra = (lid, data,result) => {
    db.query("UPDATE land SET ownername = ?,ph = ?,pottasium = ?,phosphorous = ?,nitrogen = ?,state = ?, district = ?, leaseperiod = ? WHERE lid = ? ",[data.ownername,data.ph,data.pottasium,data.phosphorous,data.nitrogen,data.state,data.district,data.leaseperiod,lid], (err,results)=> {
        if (err){
            console.log(err);
            result(err,null);
        }else{
